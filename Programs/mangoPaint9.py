...
img_origin = data.imread(PIC_NAME)

# For Scatch Mode
img = cv2.medianBlur(img_origin, BLURRED) # Blurred Image
...
fig, ax = plt.subplots(4,1,figsize=(50,100), sharex=True, sharey=True)


# 1. edge detection mode
edges = cv2.Canny(img, 70, 200, apertureSize=3)
ax[0].imshow(segmentation.mark_boundaries(img, edges, (0,0,0)))
ax[0].set_title("Edge Detection")


# 2. scatch mode
ax[1].imshow(rescale_intensity(1 - sobel_gray(img)), cmap=plt.cm.gray)
ax[1].set_xticks([]), ax[1].set_yticks([])
ax[1].set_title("Scatch mode")
